<?php
    ...
    if($typeMicrotr == 'last') {
        if($action == 'students') {
            ...
            $query_cond = '`status` = "В рассмотрении"';
        
            // INNER JOIN `staff` ON `microtraumas`.`id_staff` = `staff`.`id_staff`
            ...
            // if($status) {
            //     if($query_cond != '') {
            //         $query_cond .= " AND `status` LIKE '$status'";
            //     } else {
            //         $query_cond .= " `status` LIKE '$status'";
            //     }
            // }
...
?>
                    <div class="row-microtr">
                        <a href="/pages/admin/microtraumas_detailed/index.php?id_microtr=<?=$line['id_microtr']; ?>" class="table-row last-row">
...
<?php
        ...
        if($action == 'students') {
            ...
            if(!empty($query_cond)){
                ...
                $query_cond .= " AND `date_microtr` >= NOW() - INTERVAL 14 DAY";
                $sql= $sql.$query_cond;
                $sthqrow2 = $sthqrow2.$query_cond;
